...
import { MoreVertical, Pin, Lock, FileText, Trash2, Sparkles, BookOpen, Mic } from 'lucide-react'
  ...
  return (
    <motion.main
      ...
      style={{
        ...
        minHeight: '100dvh',
        paddingBottom: readingMode ? 24 : 100,
      }}
    >
      {/* Top bar */}
      <div
      ...
      <div className={`px-5 py-6 ${inkWave ? 'ink-wave' : ''}`} style={{ position: 'relative' }}>
        {/* Title */}
        {readingMode ? (
          <h1 className="font-serif font-semibold mb-3" style={{ fontSize: 28, lineHeight: '36px', color: 'var(--text-primary)' }}>
            {title || 'sem título'}
          </h1>
        ) : (
          <input
      ...
      <AnimatePresence>
        {audioOpen && <AudioRecorder onClose={() => setAudioOpen(false)} onSave={(t) => { handleContentChange(content + '\n\n> 🎙️ ' + t); setAudioOpen(false) }} />}
      </AnimatePresence>
    </motion.main>
  )
}
